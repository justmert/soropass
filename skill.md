...
`@stellar/stellar-sdk` is a required peer dependency (the package declares `>=12`). These examples are verified against `@soropass/core@0.1.2` with `@stellar/stellar-sdk` v16 on Node 18+. `@soropass/core` publishes ESM, CommonJS, and type declarations.
...
- Every signature is normalized to low-S. About half of Apple passkeys produce high-S signatures, and the on-chain verifier rejects those.
...
`createPasskeyKit(options)`: `mode` is `"mock"` (in-process, no network) or `"live"` (supply real `deployer`, `indexer`, `submission`, and `signer`); `rpId` is required; `rpName` and `seed` are optional, and `seed` makes the mock deterministic; `forceHighS: true` forces a high-S signature to exercise the low-S normalizer, and the result stays `PASS`.
...
To resolve an existing account without deploying, use `deriveAccountAddress({ factoryContractId, credentialId, networkPassphrase })` (offline, where `credentialId` is the raw credential bytes, `Uint8Array`) or `connect({ rpId, indexer })`.
...
kit.setWallet("passkey");
const { address } = await kit.getAddress();
```

The kit's npm package is `@creit.tech/stellar-wallets-kit` (note the dot). The `PasskeyModule` is proposed to `Creit-Tech/Stellar-Wallets-Kit` as a pull request; until it merges, the `./modules/passkey` export is not on the published kit, so install it from the PR branch. See https://docs.soropass.dev/docs/quickstart for configuration.
