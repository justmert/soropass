/* Soropass UI — shared primitives + error copy (design prototypes).
   Pure-token styling (classes from the component CSS). ES module. */
...
/* ---- helpers ----------------------------------------------------------- */
export const truncMiddle = (s, lead = 6, tail = 4) =>
...
export function Button({ variant = 'primary', children, busy, icon, ...rest }) {
  return (
...
export function AddressChip({ address, label = 'Account', showIdenticon = true }) {
...
export const StatusLine = forwardRef(function StatusLine(
  { tone = 'info', icon, children, assertive = false, tabIndex },
  ref,
) {
  return (
    <p
...
export const ERROR_COPY = {
  ...
  // CREATE · funding branch
  'create:insufficient': { t: 'Account needs XLM', m: 'Your new account needs a little XLM to activate. Fund it or use a sponsor, then continue.', a: 'Fund account' },
  // ADD DEVICE / BACKUP SIGNER
  'adddevice:cancelled': { t: 'Cancelled', m: 'You closed the passkey prompt before the backup was added. You can try again.', a: 'Try again' },
  'adddevice:unsupported': { t: 'Device not supported', m: 'This device or passkey can’t be added as a signer. Try another device or security key.', a: 'Try again' },
  'adddevice:binding': { t: 'Couldn’t add the signer', m: 'We couldn’t add this passkey to your account on-chain. Please try again.', a: 'Try again' },
  'adddevice:network': { t: 'Connection problem', m: 'We couldn’t reach the network. Check your connection and try again.', a: 'Retry' },
};

export const ErrorState = forwardRef(function ErrorState({ code = 'create:network', onRetry, statusRef }, ref) {
  const c = ERROR_COPY[code] || ERROR_COPY['create:network'];
  const [, codeKey] = code.split(':');
  return (
    <div className="pk-result" ref={ref}>
      <div className="pk-result__head">
        <span className="pk-glyph pk-glyph--error">
          <IconAlert />
        </span>
