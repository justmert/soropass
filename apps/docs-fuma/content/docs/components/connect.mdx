...
<Callout type="info">
  Architecture: the kit can't host custom module pages, so **connect rides the kit's own modal** and
...
const { signedTxXdr } = await StellarWalletsKit.signTransaction(addSignerXdr, {
  networkPassphrase,
});
