...
Install `@stellar/stellar-sdk` at version 17 or newer: `@soropass/core@0.3.0` builds against the stellar-sdk 17 XDR API, matching the kit's own stellar-sdk 17 dependency. The kit package that carries the module is `@creit.tech/stellar-wallets-kit` (note the dot).
