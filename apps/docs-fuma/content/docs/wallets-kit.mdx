...
<Callout type="info" title="Where the module lives">
  The module is built inside the kit codebase (kit v2.5.0, Deno-native, 31 kit-side tests) and ships
  ...
  suite (Chromium, Firefox, WebKit). It is proposed to the Creit Tech maintainers in
  [Stellar-Wallets-Kit issue #95](https://github.com/Creit-Tech/Stellar-Wallets-Kit/issues/95). The
  published `@creit.tech/stellar-wallets-kit@2.5.0` package does not include a `./modules/passkey`
...
Pin `@stellar/stellar-sdk` to `>=12 <17`: version 17 reworked the XDR API in a way `@soropass/core@0.2.0` does not yet support. The kit package that carries the module is `@creit.tech/stellar-wallets-kit` (note the dot).
...
const cfg = {
  ...
  factoryContractId: 'C...', // your deployed AccountFactory
};
...
const backup = await registerPasskey({ rpId: location.hostname, rpName: 'Acme Wallet', userName: 'alice backup' });
const addSignerOp = new Contract(address).call('add_signer', nativeToScVal(backup.publicKey, { type: 'bytes' }));
...
| `factoryContractId`                | `string`                            | The `AccountFactory` C-address. With it and the persisted founding public key, `getAddress` derives the account address offline from the remembered credential id: no indexer, no deploy round-trip.  |
...
| `indexer`                          | `IndexerAdapter`                    | Resolves a credential id to its deployed account(s). Required when neither `factoryContractId` nor `smartWalletDeployer` is set; otherwise the fallback when offline derivation is not possible.      |
...
- **Buffer polyfill.** The module needs a `Buffer` polyfill in your app. This is a `@stellar/stellar-sdk` requirement, the same one the kit's Ledger and Trezor modules carry.
