...
Pin `@stellar/stellar-sdk` to `>=12 <17`: version 17 reworked the XDR API in a way `@soropass/core@0.2.1` does not yet support. The kit package that carries the module is `@creit.tech/stellar-wallets-kit` (note the dot).
