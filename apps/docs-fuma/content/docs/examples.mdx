...
Desktop initiates, the phone that holds the passkey approves (hybrid / caBLE / QR). `hybrid_transport` is a documented, sourced surface ([see the matrix row](/docs/compatibility)).

</Tab>
</Tabs>

<Callout type="info">
  Previews run in **mock mode** (no network, no authenticator) because browsers restrict WebAuthn
  inside cross-origin iframes. The real ceremonies run at
  [demo.soropass.dev](https://demo.soropass.dev), every transaction linked to Stellar Expert.
</Callout>
