...
Then there is nothing for the passkey to sign for: a passkey has **no on-chain identity until its `C` contract is deployed**. "Adding passkeys" _is_ deploying that contract (step 1 above, via the [deployer](/docs/sdk/accounts#v1-smart-wallet-adapters)). Before it exists, the passkey is just a key on the device with nowhere to act.
...
<Callout type="warn">
  ...
  model](/docs/security#recovery-model). (`addSigner` enrolls additional passkey signers; the native
  account stores 65-byte secp256r1 keys.)
</Callout>

## Who pays the fees

Authorization and payment are **decoupled**: the passkey says _who can move funds_; a classic account or a relayer _pays the network fee and submits the transaction_. Deploying the contract and every passkey-signed transaction needs a fee source.
...
<Callout type="warn">
  Never ship a hot key in production. Mainnet wallets sponsor fees through a relayer (Launchtube /
  OZ Relayer) or your user's own account, not a bundled secret.
</Callout>
