...
<Callout type="warn">
**Add-device is an account-takeover path.** Adding a new signer grants it the power to authorize the account, so the add-device flow must be gated behind a fresh re-authentication with an existing signer — never an unauthenticated mutation. The styled add-device screen surfaces this requirement; enforcing the re-auth gate (and the on-chain multi-signer add) is part of the roadmap.
</Callout>
