...
<Callout type="tip">
  Not just a JS model. A real deployed `__check_auth` on Stellar testnet accepted a
  `SorobanAuthorizationEntry` assembled and signed by `@soropass/core`: the **positive** run (the account's
  registered P-256 key) succeeded and the host `secp256r1_verify` trapped the **negative** run (a
  different key). See the [on-chain proof contract](https://stellar.expert/explorer/testnet/contract/CB3IBD2JTLOFPLT4JFJ3KOIALDTKUMGLSSDZTOK7W2YWCZTPTZU66XB2)
