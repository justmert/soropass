...
<Callout type="info">
  The `PasskeyModule` implements the `@creit.tech/stellar-wallets-kit` `ModuleInterface` (v2.5.0) and is
  proposed to the kit as a pull request. Until it merges, the `./modules/passkey` export is not on the
  published kit, so install it from the PR branch. Paths A and B use published packages.
</Callout>
