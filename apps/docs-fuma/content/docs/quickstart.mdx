...
<Callout type="info">
  The `PasskeyModule` (`@soropass/wallets-kit-module`) implements the `@creit.tech/stellar-wallets-kit`
  `ModuleInterface` (v2.2.0) and runs from the workspace (`packages/kit-module`). It is **not yet
  published to npm**, so consume it from the workspace. Paths A and B use published packages.
</Callout>
