...
<Tabs items={['pnpm', 'npm']}>
  <Tab value="pnpm">```bash pnpm add @soropass/core "@stellar/stellar-sdk@>=17" ```</Tab>
  <Tab value="npm">```bash npm i @soropass/core "@stellar/stellar-sdk@>=17" ```</Tab>
</Tabs>

Install `@stellar/stellar-sdk` at version 17 or newer: `@soropass/core@0.3.0` builds against the stellar-sdk 17 XDR API (on stellar-sdk 12 through 16, install `@soropass/core@0.2.1` instead; 0.2.1 signs only classic address credentials, while 0.3.0 also signs the `addressV2` credentials Protocol 23 networks return from simulation). `@soropass/core` installs from npm and runs in the browser without a `Buffer` polyfill. Path B's `@soropass/ui` builds from the [SoroPass monorepo](https://github.com/justmert/soropass), where it lives as a pnpm workspace package.
