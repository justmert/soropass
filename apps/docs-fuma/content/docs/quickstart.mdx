...
<Cards>
  ...
  <Card title="2 · Sign" href="/docs/sdk/sign">
    Approve a transaction; the assertion is DER → raw, low-S normalized, then assembled into Soroban
    auth.
  </Card>
...
<Tabs items={['pnpm', 'npm']}>
  <Tab value="pnpm">```bash pnpm add @soropass/core ```</Tab>
  <Tab value="npm">```bash npm i @soropass/core ```</Tab>
</Tabs>
...
<Callout type="tip">
  `@stellar/stellar-sdk` is a **peer dependency** — it's never bundled into the SDK output. Install
  it alongside if your app doesn't already depend on it.
</Callout>
...
<Callout type="info">
  The `deployer` (an `AccountDeployer`) is the only piece the adopter supplies — it deploys the
  smart account for a new passkey against your factory. Everything else (DER → raw, low-S, the
  Soroban auth struct) is handled inside the SDK.
</Callout>
...
<Callout type="info">
  **Two facts to keep in mind.** `module.isAvailable()` resolves `isUVPAA` within a 500ms budget —
  that's what gates whether passkey appears in the wallet picker. And raise the resource budget
  after `simulate()`: on-chain `secp256r1_verify` is under-budgeted by simulation, so a transaction
  sent at the simulated budget will fail.
</Callout>
...
<Callout type="tip">
  The full taxonomy with per-code copy lives in [the KitError taxonomy](/docs/sdk/errors). The
  styled screens already render the right message per code.
</Callout>
...
<Callout type="tip">
  Want a styled flow instead of wiring the screens yourself? Mount one from `@soropass/ui/styled` —
  see the [component docs](/docs/components/create).
</Callout>
