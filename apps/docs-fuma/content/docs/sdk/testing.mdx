...
The complete loop against `@soropass/core@0.2.0` (with `@stellar/stellar-sdk` `>=12 <17`; version 17 reworked the XDR API `sampleAuthEntry` relies on): create an account, sign a ready-made auth entry, check the signature the way `__check_auth` does, and confirm a wrong key fails.
