...
The complete loop against `@soropass/core@0.3.0` (with `@stellar/stellar-sdk` `>=17`, the peer range `sampleAuthEntry` builds against): create an account, sign a ready-made auth entry, check the signature the way `__check_auth` does, and confirm a wrong key fails.
