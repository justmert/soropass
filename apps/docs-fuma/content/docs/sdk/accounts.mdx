...
The [`addSigner`](#addsigner) / [`removeSigner`](#removesigner) helpers below target the passkey-kit v1 smart-wallet ABI, not this account.

## The v1 smart-wallet

Beyond the native account, `@soropass/core` also targets the passkey-kit **v1 smart-wallet** (Protocol 27), a multi-signer account whose `type Signature = Signatures(Map<SignerKey, Signature>)`. The [`sendSmartWalletTx`](#sendsmartwallettx) path drives **payments**, arbitrary contract calls, and v1 signer management (`addSigner` / `removeSigner`).
...
The building blocks behind the `smart-wallet` target and the [`addSigner`](#addsigner) / [`removeSigner`](#removesigner) flows, exported from `@soropass/core/sign` for advanced use (custom multi-signer assembly, or driving the wallet contract directly).

| Function                                             | Returns                     | Description                                                                                                                                     |
| ---------------------------------------------------- | --------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- |
| `applyAssertionToSmartWalletEntry(entry, assertion)` | `SorobanAuthorizationEntry` | Sets (or merges into) the entry's `Signatures(Map<SignerKey, Signature>)` value, keyed by the assertion's credential id and sorted canonically. |
...
[`referenceSmartWalletCheckAuth`](/docs/sdk#soroban-auth-primitives) verifies the assembled multi-signer entry the way the v1 `__check_auth` does.
...
// publicKey is the founding SEC-1 key the factory `deployed` event reported, when present.
```
...
An [`AccountDeployer`](/docs/sdk#accountdeployer) turns a freshly registered passkey into a deployed smart account. Two ship with the SDK; supply your own for any other contract scheme.
...
| `smartWalletV1Deployer(options)` | The passkey-kit v1 smart wallet (see [v1 smart-wallet adapters](#v1-smart-wallet-adapters)).                                                                                                                                    |
...
The factory scheme is deterministic: [`deriveAccountAddress`](/docs/sdk#address-derivation) computes the same C-address offline from the credential id and public key, so a returning user resolves their address with no network call.
...
<Callout type="info">
  **Invariant #4**: pluggable adapters for submission + indexer; the default is zero-infra (direct +
  events). Because every factory returns the same interface, you can start on the default and move
  to Launchtube, the OZ Relayer, or Mercury later without touching call sites.
</Callout>
