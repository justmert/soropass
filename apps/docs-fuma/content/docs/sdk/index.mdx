...
<Callout type="info" title="Version">
  This reference documents `@soropass/core` 0.2.1, the npm `latest`. It pairs with the v0.2 account
  contracts: key-bound address derivation, the signer public key inline in the signature struct, and
  `userVerification` defaulting to `'required'`.
</Callout>
