...
<Callout type="info" title="Version">
  This reference documents `@soropass/core` 0.3.0, the npm `latest`, which requires
...
- **~2 runtime deps**: `@noble/curves` + `@noble/hashes`; the core barrel entry is ~3.9 KB gzip, individual subpath entries from ~0.15 KB (measured on the 0.3.0 dist).
