...
<Cards>
  <Card title="Address-credential auth" href="#signtransaction">
    Signs every address-credential Soroban auth entry carried by the InvokeHostFunction operations
    of a transaction.
  </Card>
  <Card title="Always low-S" href="#low-s-normalization">
    Every assertion is low-S-normalized downstream — Soroban's secp256r1_verify does not enforce it,
    so the SDK must.
  </Card>
  <Card title="Pluggable signer" href="#browserpasskeysigner">
    browserPasskeySigner adapts navigator.credentials.get; inject a custom WebAuthnClient for tests
    or non-browser hosts.
  </Card>
</Cards>
...
<Callout type="tip">
  Omit `allowCredentials` to drive a discoverable (resident-key) prompt where the authenticator
  surfaces the credential itself.
</Callout>
...
<Callout type="warn">
  ...
  [`sendSmartWalletTx`](/docs/sdk/smart-wallet#sendsmartwallettx) — it re-simulates the signed tx in
...
To wrap the signature as the passkey-kit smart-wallet `Signatures` map instead of the single-signer struct, pass `target: 'smart-wallet'` — see [smart-wallet & recovery](/docs/sdk/smart-wallet).
