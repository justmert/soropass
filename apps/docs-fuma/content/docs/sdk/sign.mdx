...
<Callout type="warn">
  `simulateTransaction` does not run `secp256r1_verify`, so it under-budgets `__check_auth`. Raise
  the instruction budget + resource fee before submit, or use a managed submitter.
</Callout>
