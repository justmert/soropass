...
<Callout type="info">
  The taxonomy is frozen with `Object.freeze` — `KIT_ERROR_CODES` is exhaustively switchable, so the
  compiler catches a missing case when you add handling.
</Callout>
...
<Callout type="tip">
  Unknown or unmapped codes fall back to the screen's `*:unsupported` message — a safe, device-level
