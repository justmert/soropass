...
class LedgerModule {
    ...
    async getAddress(opts) {
        ...
        try {
            ...
            return { address: stellar_sdk_1.StrKey.encodeEd25519PublicKey(result.rawPublicKey) };
        }
        catch (e) {
    ...
    async getAddresses(page = 0) {
        ...
        for (let i = 0; i < 10; i++) {
            ...
            results.push({
                publicKey: stellar_sdk_1.StrKey.encodeEd25519PublicKey(result.rawPublicKey),
                index: i + startIndex,
            });
        }
    ...
    async signTransaction(xdr, opts) {
        ...
        if (opts?.path) {
            ...
            const result = await str.getPublicKey(mnemonicPathValue);
            account = stellar_sdk_1.StrKey.encodeEd25519PublicKey(result.rawPublicKey);
        }
