 ...
 */
function transformTransaction(path, tx) {
    ...
    const operations = tx.operations.map((o, i) => {
        ...
        if (operation.type === "allowTrust") {
            operation.assetType = transformAsset(new stellar_sdk_1.Asset(operation.assetCode, operation.trustor)).type;
        }
        if (operation.type === "manageData" && operation.value) {
            operation.value = (0, mod_js_1.encodeHex)(operation.value);
        }
...
function transformSigner(signer) {
    ...
    if (signer.ed25519PublicKey) {
        key = (0, mod_js_1.encodeHex)(stellar_sdk_1.Keypair.fromPublicKey(signer.ed25519PublicKey).rawPublicKey());
    }
    if (signer.preAuthTx instanceof Uint8Array) {
        type = 1;
        key = (0, mod_js_1.encodeHex)(signer.preAuthTx);
    }
    if (signer.sha256Hash instanceof Uint8Array) {
        type = 2;
        key = (0, mod_js_1.encodeHex)(signer.sha256Hash);
    }
...
function transformMemo(memo) {
    switch (memo.type) {
        ...
        case stellar_sdk_1.MemoHash:
            return { type: 3, hash: (0, mod_js_1.encodeHex)(memo.value) };
        case stellar_sdk_1.MemoReturn:
            return { type: 4, hash: (0, mod_js_1.encodeHex)(memo.value) };
        default:
            return { type: 0 };
    }
}
