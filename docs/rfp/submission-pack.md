...
| ----------------------------------------------------------------------------------------------------------------- | --------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **External coordination — kit maintainers** must accept the upstream PR (the "adopted, not parallel" requirement) | **#1**          | De-risk by arriving with a finished branch + demo + design doc (S16/S17) and an existing conversation thread (Issue #90, S25). The SDK is independently useful even if adoption slips. |
...
| **Compatibility guide that stays fresh**                   | sourced matrix + virtual-auth CI + weekly cron + dated snapshots                      | S07/S09/S10; `apps/matrix/`, `.github/workflows/ci.yml`; `/matrix`                 |
...
| **Adopted into stellar-wallets-kit (not parallel)**        | `PasskeyModule implements ModuleInterface` on a kit branch; upstream PR pending       | S16/S17 `packages/kit-module/`; S27 (human)                                        |
...
| **Reference integration + demo**                           | demo through the kit + recording                                                      | S21 `apps/demo/` (recording pending, human)                                        |
...
| **Maintainer coordination ("evidence of a conversation")** | integration design doc + Issue #90 thread                                             | S16 `docs/integration/stellar-wallets-kit.md`; S25 (human)                         |
