...
import { Account, Address, Networks, Operation, TransactionBuilder, xdr } from '@stellar/stellar-sdk';
...
function twoEntryTx(): string {
  ...
  return new TransactionBuilder(new Account(SOURCE_G, '0'), { fee: '100', networkPassphrase: NETWORK })
    .addOperation(op)
    .setTimeout(60)
    .build()
    .toXDR();
...
function authEntries(signedXdr: string): xdr.SorobanAuthorizationEntry[] {
  return xdr.TransactionEnvelope.fromXDR(signedXdr, 'base64')
    .v1().tx().operations()[0].body().invokeHostFunctionOp().auth();
}
...
describe('signAuthEntry / signTransaction hardening', () => {
  it('low-S normalizes an already-compact high-S signature (invariant #2, H2)', async () => {
    // A real, valid high-S signature (S > n/2): sign, then replace S with n - S.
    const low = p256.sign(new Uint8Array(32).fill(3), p256.utils.randomPrivateKey());
    ...
    const map = xdr.SorobanAuthorizationEntry.fromXDR(signedXdr, 'base64')
      .credentials().address().signature().map()!;
    const outSig = new Uint8Array(
      map.find((e) => e.key().sym().toString() === 'signature')!.val().bytes(),
    );
